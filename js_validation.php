...
<body>
  <form>
    <table border="1" width="600px" align="center">
      <tr height="100px">
        <td colspan="3" align="center"><h2 id="headerText">PERSON PROFILE</h2></td>
      </tr>
      <tr height="40px">
      ...
      <tr height="40px">
        ...
        <td width="350px"> <input type="text" name="" id='IdInputId'> <br>
        </td>
        <td class="valStatus">Validation Status</td>
      </tr>
      <tr height="40px">
    ...
    <script type="text/javascript">
      ...
      //check if it is a alphabet (A-Z // a-z) // checks single character
      ...
      //checks a word/sentence // (a-z // A-Z // space ' ' , '-'' , '.'')
      ...
      //checks a word/sentence // (a-z // A-Z)
      function checkValidCharForEmailExtention(value){
        for(var i=0; i<value.length; i++){
          var val = value.charCodeAt(i);        
          switch(true)
          {
      ...
      //checks a word/sentence for email // space ' ' , '-' , '.' , '@' , '_'
      function checkValidCharForEmail(value){
        for(var i=0; i<value.length; i++){
          ...
          switch(true)
          {
            ...
            console.log('a-z');
            continue;
            case (val>=48 && val<=57): //0-9
            console.log('number 0-9');
            continue;
            case (val == 32 || val == 45 || val == 46 || val == 64 || val == 95): // 
            console.log('Other caracter');
            continue;
            default:
            return false;
      ...
      //array element length check for email extention[.com part]
      function checkEmailExtentionLength(fullName){
      ...
      //id validation
      function idVal(){
        var obj = document.getElementById('IdInputId');
        var value = obj.value.trim();
        var BlankTd =  obj.parentNode.parentNode.lastElementChild;
        
        if(value=="")
        {
          BlankTd.innerHTML = 'id can not be empty';
          BlankTd.style.background = 'red';
          BlankTd.style.color = 'white';
          return;
        }
        else if(isNaN(value)){
          BlankTd.innerHTML = 'id should be a number';
          BlankTd.style.background = 'red';
          BlankTd.style.color = 'white';
          return;
        }
          ...
          if(!Number.isInteger(value)){
            BlankTd.innerHTML = 'integer only';
            BlankTd.style.background = 'red';
            BlankTd.style.color = 'white';
            return;
          }
          ...
          else if(responseT == 'space')
          {
            ...
            if(valueSplitOnAtTheRateOf[1].indexOf('.')>-1){
              ...
              if(valueSplitOnDot.length>1){
                var responseT =  checkEmailExtentionLength(valueSplitOnDot);
                var responseValidationOfDigit = checkValidCharForEmailExtention(valueSplitOnDot[1]);
                ...
                else if(responseValidationOfDigit == false){
                  BlankTd.innerHTML = 'extention cant contain number';
                  BlankTd.style.background = 'red';
                  BlankTd.style.color = 'white';
                  return;  
                }
                else if(responseT == 'long')
                {
                  BlankTd.innerHTML = 'extention too long';
                  BlankTd.style.background = 'red';
                  BlankTd.style.color = 'white';
                  return;  
                }
      ...
      function validateDOB(){
        ...
        if(month.value < 0)
        {
          BlankTd.innerHTML = "month negetive vulue not allowed";
          BlankTd.style.background = 'red';
          BlankTd.style.color = 'white';
        }
        else if(month.value == "")
        ...
        if(day.value !="")
        {
          if(day.value < 0)
          {
            BlankTd.innerHTML = "day negetive vulue not allowed";
            BlankTd.style.background = 'red';
            BlankTd.style.color = 'white';
          }
          else if(leapYearStatus==false && month.value == 2 && day.value > 28)
          {
     ...
     function validateAll(){
      ...
      let blood = valBloodGroup();
      let degree = valDegree();
      let dob = validateDOB();
      let id = idVal();

      console.log(name+email+gender+blood+degree+dob+id);

      if(name=='valid' && email=='valid' && gender == 'valid' && blood == 'valid' && degree == 'valid' && dob=='valid' && id=='valid')
      {
        console.log('inside');
        var i = document.getElementById('headerText');
